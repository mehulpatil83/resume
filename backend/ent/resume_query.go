...
func (_q *ResumeQuery) QueryGenerations() *GenerationQuery {
	query := (&GenerationClient{config: _q.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := _q.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (_q *ResumeQuery) Clone() *ResumeQuery {
	...
	return &ResumeQuery{
		...
		withGenerations: _q.withGenerations.Clone(),
...
func (_q *ResumeQuery) WithGenerations(opts ...func(*GenerationQuery)) *ResumeQuery {
	query := (&GenerationClient{config: _q.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	_q.withGenerations = query
	return _q
}
...
func (_q *ResumeQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Resume, error) {
	...
	if query := _q.withGenerations; query != nil {
		if err := _q.loadGenerations(ctx, query, nodes,
			func(n *Resume) { n.Edges.Generations = []*Generation{} },
			func(n *Resume, e *Generation) { n.Edges.Generations = append(n.Edges.Generations, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (_q *ResumeQuery) loadGenerations(ctx context.Context, query *GenerationQuery, nodes []*Resume, init func(*Resume), assign func(*Resume, *Generation)) error {
	...
	query.Where(predicate.Generation(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(resume.GenerationsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
	...
	for _, n := range neighbors {
		fk := n.resume_generations
		if fk == nil {
			return fmt.Errorf(`foreign-key "resume_generations" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "resume_generations" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
