...
func (_q *UserQuery) loadResumes(ctx context.Context, query *ResumeQuery, nodes []*User, init func(*User), assign func(*User, *Resume)) error {
	...
	for _, n := range neighbors {
		fk := n.user_resumes
		if fk == nil {
			return fmt.Errorf(`foreign-key "user_resumes" is nil for node %v`, n.ID)
		}
		node, ok := nodeids[*fk]
		if !ok {
			return fmt.Errorf(`unexpected referenced foreign-key "user_resumes" returned %v for node %v`, *fk, n.ID)
		}
		assign(node, n)
	}
	return nil
